...
func (v *ExpensiveView) Renderable(ctx context.Context) (*View, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	case err := <-v.Err:
		return nil, err
	case data := <-v.Data:
		return R(Template{Tpl: expensiveViewTpl, Data: data}), nil
	}
}
...
func (v ViewWithTimeout) Renderable(ctx context.Context) (*View, error) {
