...
func (e nodeChildren) AsHTML(ctx context.Context, t tag) (template.HTML, error) {
	content, err := view.Render(ctx, view.Views(e))
	if err != nil {
		return content, fmt.Errorf("nodechildren: %w", err)
	}

	return t.opening() + content + t.closing(), nil
}

type void struct{}

func (v void) AsHTML(_ context.Context, t tag) (template.HTML, error) {
	return t.opening(), nil
}
